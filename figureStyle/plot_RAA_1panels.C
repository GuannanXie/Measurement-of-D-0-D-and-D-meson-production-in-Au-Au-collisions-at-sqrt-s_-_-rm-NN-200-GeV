void plot_RAA_1panels()
{
   ...
   const Int_t NC = 3;
   const Int_t NPt = 11;
   double pT[NC][NPt], y[NC][NPt], ye[NC][NPt], yesh[NC][NPt], yesl[NC][NPt];
   const char nameCent[3][250] = {"0_10", "10_40", "40_80"};
   const Char_t *txt[NC] = {"0-10%", "10-40%", "40-80%"};
   float Nbin[3] = {938.80170, 386.08527, 56.99229};
   float NbinErr[3] = {26.28048, 29.86811, 14.85246 };


   TFile *fin = new TFile("rootfiles/D0RAA_Run14HFT.root");
   TGraphErrors *gr_stat_tmp[NC];
   TGraphAsymmErrors *gr_sys_tmp[NC];
   for (int i = 0; i < NC; i++)
   {
      gr_stat_tmp[i] = (TGraphErrors *)fin->Get(Form("D0_RAA_err_%s", nameCent[i]));
      gr_sys_tmp[i] = (TGraphAsymmErrors *)fin->Get(Form("D0_RAA_sys_combine_%s", nameCent[i]));
      gr_stat_tmp[i]->Print();
      ...
      for (int j = 0; j < NPt; j++)
      {
         ...
         yesh[i][j] = gr_sys_tmp[i]->GetEYhigh()[j];
         yesl[i][j] = gr_sys_tmp[i]->GetEYlow()[j];
      }
   }
   ...
   TLegend *leg = new TLegend(0.65, 0.69, 0.93, 0.85);
   leg->SetFillStyle(0);
   leg->SetLineStyle(0);
   leg->SetLineColor(10);
   leg->SetLineWidth(0.);
   ...
   for (int i = 0; i < NC - 1; i++)
   {
      TBox *bx = new TBox(x2 - (NC - i) * 0.15, (1. - NbinErr[i] / Nbin[i]), x2 - (NC - i - 1)  * 0.15, (1. + NbinErr[i] / Nbin[i]));
      bx->SetFillColor(kColor[i]);
      bx->Draw();
   }
   const float ppnormsys = sqrt(0.081 * 0.081 + 0.052 * 0.052); //pp normalization 0.081%
   TBox *bx = new TBox(x2 - 0.15, (1. - ppnormsys), x2, (1. + ppnormsys));
